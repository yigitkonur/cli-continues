...
| Antigravity | Medium | The parser now indexes `.gemini/antigravity/conversations/*.pb`, `brain/<id>/`, state summaries, and optional live RPC. Offline raw `.pb` transcript schema is still private, so full message fidelity requires live RPC. |
