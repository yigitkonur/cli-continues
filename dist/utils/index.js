...
export async function buildIndex(force = false) {
    ...
    const [codexSessions, claudeSessions, copilotSessions, geminiSessions, opencodeSessions] = await Promise.all([
        parseCodexSessions(),
        parseClaudeSessions(),
    ...
    const allSessions = [...codexSessions, ...claudeSessions, ...copilotSessions, ...geminiSessions, ...opencodeSessions];
 ...
 */
export async function getAllSessions(forceRebuild = false) {
    return buildIndex(forceRebuild);
}
/**
 * Get sessions filtered by source
 */
export async function getSessionsBySource(source, forceRebuild = false) {
 ...
 */
export async function extractContext(session) {
 ...
 */
export function getCachedContext(sessionId) {
...
export function formatSession(session) {
    const source = `[${session.source.padEnd(7)}]`;
 ...
 */
export function sessionsToJsonl(sessions) {
