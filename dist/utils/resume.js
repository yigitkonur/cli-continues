 ...
 */
export async function nativeResume(session) {
 ...
 */
export async function crossToolResume(session, target) {
 ...
 */
export async function resume(session, target) {
 ...
 */
function runCommand(command, args, cwd, stdinData) {
 ...
 */
export function getResumeCommand(session, target) {
    const actualTarget = target || session.source;
    if (actualTarget === session.source) {
        switch (session.source) {
            case 'codex':
