...
program
  ...
  .action(async (options) => {
    try {
      ...
      const { selectedSession } = await inquirer.prompt<{ selectedSession: UnifiedSession }>([
        {
          type: 'list',
      ...
      const { targetTool } = await inquirer.prompt<{ targetTool: SessionSource }>([
        {
          type: 'list',
          name: 'targetTool',
...
program
  ...
  .action(async (sessionId, options) => {
    try {
      ...
      const target = options.target as SessionSource | undefined;
